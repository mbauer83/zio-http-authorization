...
class AuthorizeByTenantIdOrSuperRolePolicySpec extends ZSuite:
  ...
  testZ("AuthorizeByTenantIdOrSuperRolePolicy must authorize user with any tenant-id and SUPER role.") {
    val someUser02 = MockUser("someUserId2", Some("someTenantId2"), Set(SUPER), Set.empty[Permission])
    for {
  ...
  val someUser03 = MockUser("someUserId2", Some("someTenantId2"), Set.empty[Role], Set.empty[Permission])
  for {
